...
| **FMT-10** | Los marcadores salen en el mismo formato en que **Pro Tools escribe los suyos**: cabecera de sesión, formato de timecode y las columnas `#`, `LOCATION`, `TIME REFERENCE`, `UNITS`, `NAME`, `COMMENTS`. | ✅ |
...
**Automáticamente**, en `pruebas/formatos.prueba.js`, con la cadena completa:
leer un SRT, construir el libreto, y volverlo a exportar. Lo único simulado es
lo ajeno al asunto —el reloj del vídeo y el guardado—; el resto es código real,
incluido el descargador, para que el nombre del archivo también cuente.

Lo que se comprueba de verdad: los timecodes en sus cuatro formas y que
escribir y volver a leer **devuelva el mismo instante**; el CSV con comillas
dobladas y saltos de línea dentro de un campo; un STL **fabricado byte a byte**
en la propia prueba, con su código de color que hay que ignorar y su juego de
caracteres que hay que denunciar; la suma y la resta del TC de inicio; y la ida
y vuelta del SRT.

Y **a mano**, los casos construidos en la propia página:
...
- **FMT-9** y **FMT-N2** —que importar avise antes de sustituir el libreto— no
  están probadas: viven en `ioImportar`, que abre un aviso en pantalla y espera
  una respuesta. Lo probable es que haya que separar la decisión del diálogo
  para poder comprobarla.
- **FMT-10**: la forma del archivo de marcadores **sí** está comprobada
  (cabecera, columnas y timecode de reloj), pero **no se ha importado nunca en
  Pro Tools**. Que el formato sea el correcto sigue siendo una intención hasta
  que alguien lo pruebe en la sala. Una prueba automática no puede decidir eso.
- El lector de guiones de Word y PDF no está cubierto aquí: eso pertenece a
  [`01-libreto.md`](01-libreto.md), que sigue siendo la mayor laguna.
